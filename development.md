...
In that case, you have two options: you could use the instructions to modify your system `hosts` file with the instructions below in **Development with a custom IP** or you can just use `localhost.jdpillaris.com`, it is set up to point to `localhost` (to the IP `127.0.0.1`) and all its subdomains too. And as it is an actual domain, the browsers will store the cookies you set during development, etc.

If you used the default CORS enabled domains while generating the project, `localhost.jdpillaris.com` was configured to be allowed. If you didn't, you will need to add it to the list in the variable `BACKEND_CORS_ORIGINS` in the `.env` file.

To configure it in your stack, follow the section **Change the development "domain"** below, using the domain `localhost.jdpillaris.com`.

After performing those steps you should be able to open: http://localhost.jdpillaris.com and it will be served by your stack in `localhost`.
...
```
DOMAIN=localhost.jdpillaris.com
```
...
Frontend: http://localhost.jdpillaris.com

Backend: http://localhost.jdpillaris.com/api/

Automatic Interactive Docs (Swagger UI): http://localhost.jdpillaris.com/docs

Automatic Alternative Docs (ReDoc): http://localhost.jdpillaris.com/redoc

Adminer: http://localhost.jdpillaris.com:8080

Traefik UI: http://localhost.jdpillaris.com:8090
